...
var map = {};
var reg_set = /^\s*set|setx\s+(.*?)\s*=\s*(['"`]*)([\s\S]*)\2$/im
var reg_call = /^\s*call\s+(["]?)(.*?)\1([\s\S]*)$/i;
var reg_for = /^\s*for\s+([%\w]+)\s+in\s*\((.*?)\)\s*do\s+(.*?)$/i
var env = process.env;
var call = function (file,args = []) {
    gbk2utf8(
        ...
        .replace(/%~dp\d/ig, path.resolve(path.parse(file).dir))
        .split(/[\r\n]+/g)
        .forEach(get);
};
