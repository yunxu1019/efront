var getBrowserTypeFromUserAgent = function () {
    ...
    if (/TheWord/.test(navigator.userAgent)) return "sjzc";
    // 火狐浏览器
    // Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0 
    if (/Firefox/.test(navigator.userAgent)) return 'ffox';
    // Edge
    // Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Safari/537.36 Edg/107.0.1418.56
    if (/Edg[e\/]/.test(navigator.userAgent)) return 'edge';
    // ie8
    ...
    if (/Presto/.test(navigator.userAgent)) return 'prst';
    // QQ浏览器
    // Mozilla/5.0 (Windows NT 10.0; WOW64; Trident/7.0; rv:11.0) like Gecko Core/1.94.172.400 QQBrowser/11.1.5140.400 
    if (/QQBrowser/.test(navigator.userAgent)) return 'qqbr';
    if (/QQ/.test(navigator.userAgent)) return 'qqqq';
    // 搜狗浏览器
    ...
    if (/MetaSr/.test(navigator.userAgent)) return 'mtsr';
    // 微信
    ...
    if (/MicroMessenger/.test(navigator.userAgent)) return 'mmsg';
    // 猎豹浏览器
    ...
    if (chrome) return +chrome[1] < 108 ? "360s" : "chrm";
    // Safari
    // Mozilla/5.0 (Windows NT 6.2; WOW64) AppleWebKit/534.57.2 (KHTML, like Gecko) Version/5.1.7 Safari/534.57.2
    if (/Safari/.test(navigator.userAgent)) return 'sfri';
    return ["ayou", 'lbbr'][Math.random() * 2 | 0];
};
var type = getBrowserTypeFromUserAgent();
return {
    name: '',
    type,
    id: '',
